module TelegramBot
    ...
    class Bot
        ...
        def set_webhook(url, allowed_updates: %i(message))
            logger.info "Setting webhook url to #{url}, allowed_updates #{alloweb_updates}"
            request = WebhookRequest.new(url: url, allowed_updates)
        ...
        def get_last_updates(opts = {})
            opts[:offset] ||= @offset
            request = UpdateRequest.new(opts)
